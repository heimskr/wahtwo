...
namespace Wahtwo {
	...
	INotifyWatcher::INotifyWatcher(std::vector<std::string> paths, bool subfiles):
		paths(std::move(paths)), subfiles(subfiles) {
	...
	void INotifyWatcher::start() {
		...
		for (;;) {
			...
			if (FD_ISSET(fd, &read_set)) {
				...
				std::filesystem::path path = std::filesystem::path(wd_path) / std::string(reinterpret_cast<const char *>(event) + offsetof(inotify_event, name));

				if (subfiles) {
					if ((mask & IN_DELETE) != 0 && (mask & IN_ISDIR) != 0) {
						std::string canonical = std::filesystem::weakly_canonical(path);
					...
					if ((mask & IN_CREATE) != 0 && (mask & IN_ISDIR) != 0) {
						std::string canonical = std::filesystem::weakly_canonical(path);
						int wd = inotify_add_watch(fd, canonical.c_str(), MASK);
						if (wd == -1) {
							throw Error("inotify_add_watch failed", errno);
						}
						watchDescriptors.emplace(canonical, wd);
						watchPaths.emplace(wd, std::move(canonical));
					}
				}
